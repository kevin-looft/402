...
public class Recursion {
	...
	public static double sum (double[] a) {
		double start = (a.length > 0) ? a[0] : 0;
		return sumHelper(a, 1, start); 
	}
	...
	public static double sumHelper (double[] list, int i, double result) {
		...
		if (i < list.length) {
			return result += sumHelper(list, i + 1, list[i]);
		}
	...
	public static void reverse (double[] a) {
		int hi = a.length - 1;
		int low = 0;
		reverseHelper(a, hi, low);
	}
	
	public static void reverseHelper(double[] a, int hi, int low) {
		if (low < hi) {
			double loVal = a[low];
			double hiVal = a[hi];
			a[hi] = loVal;
